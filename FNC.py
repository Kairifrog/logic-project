...
def Clausula(C):
    ...
    while len(C) > 0:
        workSymbol = C[0]
        if workSymbol == "-":
            clause_list.append(workSymbol + C[1])
            C = C[3:]
        else:
            clause_list.append(workSymbol)
            C = C[2:]
...
def formaClausal(A):
    ...
    while len(A) > 0:
        if (count >= len(A)):
            clauseForm_list.append(Clausula(A))
            A = []
        else:
            if A[count] == "Y":
                clauseForm_list.append(Clausula(A[:count]))
                A = A[count + 1:]
                count = 0
            else:
                count += 1

    return clauseForm_list